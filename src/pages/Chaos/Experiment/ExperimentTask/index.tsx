...
import http from 'utils/request/myRequest'
import {calculatePreCPU} from 'utils/libs/sre-utils-calculate/cpu'
...
export default function ExperimentTask() {
  ...
  const [myMetrics, setMyMetrics] = useState<IMyMetrics>({});
  const [myPreMetrics, setPreMyMetrics] = useState<IMyMetrics>({});
    ...
    if (!_.isEmpty(experimentTask)) {
      ...
      dispatch.pageHeader.setHeaderExtra(
        <Button.Group style={{float: 'right'}}>
          <Button type="normal" loading={reRunLoading}
                  disabled={(!isFinished || !handleIsAdmin(experimentTask?.permission as number, 4))}
                  onClick={handleRestartTask}><Translation>Do it again</Translation></Button>
          <Button type="primary" className={styles.buttonSecond} onClick={handleCheckDetail}><Translation>See
            details</Translation></Button>
          {!isFinished &&
            <Button type="primary" disabled={(isStopping || !handleIsAdmin(experimentTask?.permission as number, 4))}
                    warning={!isStopping}
                    onClick={handleStopTask}>{isStopping ? i18n.t('Stoppings').toString() : i18n.t('Termination').toString()}</Button>}
        </Button.Group>,
      );
    }
  ...
  useEffect(() => {
    ...
    if (!isFinishedTsk) {
      http.get('api/v1/query', {
        params: {
          query: 'node_cpu_seconds_total{cpu="0"}[33s]'
        }
      }).then(res => {
        pushPreMetric('CPU', calculatePreCPU(res.data.result))
      })
      console.log("开启轮询")
      timer = setInterval(async () => {
        const res = await http.get('api/v1/query', {
          params: {
            query: '(1-sum(increase(node_cpu_seconds_total{cpu="0",mode="idle"}[6s])) by (job)/sum(increase(node_cpu_seconds_total{cpu="0"}[6s])) by(job))*100'
          }
        })
        pushMetric('CPU', res.data.result[0])
      }, 3000)
    }
    return () => {
      if (timer !== null) {
  ...
  function pushMetric(type: string, metric: IResultItem) {
    if (Object.keys(myMetrics).includes(type) && myMetrics[type]) {
      if (myMetrics[type].length >= 8) {
        myMetrics[type].shift()
      }
      myMetrics[type].push(metric)
    } else {
      myMetrics[type] = [metric]
    }
    setMyMetrics({...myMetrics})
  }

  function pushPreMetric(type: string, metric: IResultItem[]) {
    myPreMetrics[type] = metric
    setMyMetrics({...myMetrics})
  }
  ...
  const renderDetail: any = (value: any, index: number, record: any) => {
    ...
    return !_.isEmpty(tolerance) && tolerance.map((tole: IToleranceValue, idx: number) => {
      return <span
        className={styles.tolerance}>{tole.name}{tole.value + tole.unit}{idx !== tolerance.length - 1 ? '；' : null}</span>;
    });
  };
  ...
  function renderDialogFooter() {
    ...
    if (source === 1) {
      return <Button.Group>
        <Button type="primary" onClick={() => handleSubmitFeebBack(true)}><Translation>OK, return to strong and weak
          dependency governance</Translation></Button>
        <Button type="normal" className={styles.normalBtn} onClick={() => handleSubmitFeebBack(false)}><Translation>Ok,
          stay on this page</Translation></Button>
      </Button.Group>;
    }
    if (source === 2) {
      return <Button.Group>
        <Button type="primary" onClick={() => handleSubmitFeebBack(true)}><Translation>OK, return to message
          walkthrough</Translation></Button>
        <Button type="normal" className={styles.normalBtn} onClick={() => handleSubmitFeebBack(false)}><Translation>Ok,
          stay on this page</Translation></Button>
        <Button type="normal" className={styles.normalBtn} onClick={() => setDependenceVisible(false)}><Translation>Cancel,
          next feedback</Translation></Button>
      </Button.Group>;
    }
    if (source === 3) {
      return <Button.Group>
        <Button type="primary" onClick={() => handleSubmitFeebBack(true)}><Translation>OK, return to disaster recovery
          drill</Translation></Button>
        <Button type="normal" className={styles.normalBtn} onClick={() => handleSubmitFeebBack(false)}><Translation>Ok,
          stay on this page</Translation></Button>
        <Button type="normal" className={styles.normalBtn} onClick={() => setDependenceVisible(false)}><Translation>Cancel,
          next feedback</Translation></Button>
  ...
  return <div className={styles.warper}>
    ...
    <span style={{fontSize: '14px'}}>指标监控</span>
    <div>
      {
        Object.keys(myMetrics).map((value) => {
          return <div key={value}>
            <MyChart preData={myPreMetrics[value]} data={myMetrics[value]} type={value}/>
          </div>
        })
      }
    </div>
    ...
    <div className={styles.taskDetail}>
      ...
      <div className={styles.taskContent}>
        <div className={styles.flows}>
          {
            loading ?
              <div className={styles.flowLoading}><Icon type='loading' size='xl'/></div> : _.isEmpty(experimentTask) ?
                <div><Translation>No data</Translation></div> : <TaskFlow
                  data={experimentTask!}
                  selectNode={activity!}
    ...
    {(feedbackStatus === 1 || isFeedbackStatus) && <div className={styles.feedBack}>
      ...
      <div className={styles.feedItem}>
        ...
        <div
          className={styles.value}>{businessStatus ? i18n.t('Influence').toString() : i18n.t('Does not affect').toString()}</div>
      </div>
      <div className={styles.feedItem}>
        <div className={styles.label}><Translation>In conclusion</Translation></div>
        <div
          className={styles.value}>{expectationStatus ? i18n.t('In line with expectations').toString() : i18n.t('Not as expected').toString()}</div>
      </div>
      {
