...
class ExperimentDetail extends BaseModel {
  ...
  setExperiment(payload: IExperimentDetailState['experiment']) {
    ...
    if (!_.isEmpty(payload)) {
      ...
      const newExperiment = {
        experimentId: (payload as IExperimentDetailState['experiment']).experimentId, // id在最外层放1个
        flow: {
          experimentId: (payload as IExperimentDetailState['experiment']).experimentId,
