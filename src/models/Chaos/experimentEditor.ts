...
class ExperimentEditor extends BaseModel {
  ...
  setExperiment(payload: IExperimentEditorState['experimentData']) {
    ...
    if (!_.isEmpty(payload)) {
      ...
      const newExperiment = {
        experimentId: (payload as IExperimentEditorState['experiment']).experimentId, // id在最外层放1个
        flow: {
          experimentId: (payload as IExperimentEditorState['experiment']).experimentId,
